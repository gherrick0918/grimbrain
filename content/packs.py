...
def load_packs(names: Sequence[str], root: str | Path | None = None) -> Dict[str, dict]:
    """Load JSON sidecars from ``packs/<name>`` directories.

    ``names`` is a list of pack names. ``root`` defaults to the repository
    root (one level above this file).
    Returns a catalog mapping normalized monster names to their JSON data.
    When multiple files share the same (name, source) pair, the entry with
    longer total action text is kept.
    """
